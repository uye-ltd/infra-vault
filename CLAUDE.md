...
- **GHCR for vault-unseal image** — built in CI with Buildah + fuse-overlayfs (daemonless OCI builds; `--isolation=chroot` for `RUN` instructions — Ubuntu 24.04 enables `apparmor_restrict_unprivileged_userns=1` by default, which blocks user-namespace creation unless `userns` is added to the AppArmor profile; chroot avoids this without weakening the runner security boundary); runner container requires `cap_add: [CHOWN, DAC_OVERRIDE, FOWNER, SETUID, SETGID]` to unpack Alpine layers; cosign-signed by digest with keyless OIDC; infra-runner deployer verifies before deploying; local bootstrap uses `build:` for first run. All GitHub Actions `uses:` references are SHA-pinned.
