...
- **GitOps deployment via [infra-runner](https://github.com/uye-ltd/infra-runner) plugin system** — no deploy job in CI; `.infra-runner.plugin` registers this repo as a plugin; the deployer polls GHCR every 60s, verifies the cosign signature, restarts `vault-unseal`, and runs `scripts/apply-policies.sh` as a post-deploy hook every cycle. `docker-compose.infra-runner.yml` is a Compose overlay that mounts this repo and the plugin descriptor into the deployer container — activate by appending it to `COMPOSE_FILE` in infra-runner's `.env`
- **Self-hosted GitHub Actions runner** — provided by [infra-runner](https://github.com/uye-ltd/infra-runner); runs on the server, connects outbound to GitHub; no SSH keys or WireGuard secrets in GitHub
...
- **Tools baked into the self-hosted runner image** — Both Vault CLI and cosign are installed during image build (which runs on GitHub-hosted runners) rather than downloaded at job runtime. Runtime downloads from these CDNs fail from the server's IP: HashiCorp's CloudFront WAF blocks the request with HTTP 404 (confirmed by `x-amzn-waf-reason: geo` response header); GitHub's release CDN times out on SSL handshake. The `cosign-installer` action triggers the latter: it first downloads a bootstrap cosign version to verify the requested version's signature, and that bootstrap download timed out, leaving a zero-byte non-executable file — causing exit 126 when the action tried to run it. The fix removes the `cosign-installer` step entirely; cosign is pre-installed at `/usr/local/bin/cosign` via the Dockerfile. The validate job installs Vault via APT at runtime (GitHub-hosted runners are not geo-blocked). Vault CE is available through 2.x; an older "CE capped at 1.17.x" claim was incorrect.
...
The `VAULT_TOKEN` (ops policy token from `make init`) is stored in infra-runner's `.env` on the server. See [infra-runner](https://github.com/uye-ltd/infra-runner)'s vault integration setup.
